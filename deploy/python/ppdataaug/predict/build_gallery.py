...
class GalleryBuilder(object):
    ...
    def __init__(self, config, predict_rec):

        self.config = config
        self.predict_rec = predict_rec
    ...
    def build(self, config):
        ...
        if operation_method in ["remove", "append"]:
            ...
            assert index.ntotal == len(
                ids.keys()), "data number in index is not equal in in id_map"
        else:
        ...
        if operation_method != "remove":
            ...
            ids_now = (np.arange(0, len(gallery_images)) + start_id).astype(
                np.int64)
    ...
    def _extract_features(self, gallery_images, config):
        ...
        for i, image_file in enumerate(tqdm(gallery_images)):
            ...
            if (i + 1) % batch_size == 0:
                results = self.predict_rec.predict_images(batch_img)
                rec_feat = [item['feature'] for item in results]
